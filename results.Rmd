...
## Loading MinYS data
```{r,echo=T}
tab.minys = read.table("quast/MinYS/transposed_report.tsv",sep="\t",comment.char = "",header=T,quote = "\"")
...
```{r,echo=T}
tab.megahit <- read.table("quast/Megahit/transposed_report.tsv",sep="\t",comment.char = "",header=T,quote = "\"")
...
```{r}
tab.metacompass <- read.table("quast/Metacompass/transposed_report.tsv",sep="\t",comment.char = "",header=T,quote = "\"")
...
```{r}
tab.minia = read.table("quast/minia/transposed_report.tsv",sep="\t",comment.char = "",header=T,quote = "\"")
...
length.tab = aggregate(tab.tmp$perc_ref,by=list(tab.tmp$Genome,tab.tmp$Method),FUN=mean)
...
perc_lengths = aggregate(tab.tmp$perc_ref,by = list(tab.tmp$Genome,tab.tmp$Method),FUN=mean)
