#!/usr/bin/env python
# -*- coding: utf-8 -*-
'''Program to generate deletions of given sizes at random positions in a given genome. C. Lemaitre'''


# 11/01/2019
# C. Lemaitre: modified from make_deletions.py + 1-based positions.
...
def usage():
    ...
    print "  -l: list of deletion sizes, separated by \',\', default=100 (1 deletion of size 100)"
...
def main():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hg:l:o:s:Nb", ["help", "genome=", "list=", "output=", "sep=", "enableN", "bed"])
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            size_list = [int(i) for i in arg.split(",")]
        elif opt in ("-s", "--sep"):
            sep = int(arg)
            if sep<0:
                sep=0
