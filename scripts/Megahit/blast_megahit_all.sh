...
for ref in  myzus # LSR1 rearranged schyzaphis
  ...
  for file in $(ls $fofDir)
      ...
      Rscript $scriptDir/filter_blast.R $assemblyDir/$ref.blast $assemblyDir/final.contigs.fa $assemblyDir/megahit.$file.$ref.fa
      ...
      Rscript $scriptDir/filter_blast.R $assemblyDir/megahit.$file.$ref.plasmids.tsv $assemblyDir/megahit.$file.$ref.fa $assemblyDir/megahit.$file.$ref.noplasmid.fa
  done
done
